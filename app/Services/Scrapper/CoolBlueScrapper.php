...
{
    const BASE_URL = 'https://openshop.uz';
   // class == button button--order button--full-width
    /**
    ...
    public function selectCategory(string $url): void
    {
       ...
       // $driver = $this->driver->findElements(WebDriverBy::className("button button--order button--full-width")->click());
    ...
    public function parseProducts(): Collection
    {
        ...
        try {
            $elements = $this->driver->findElements(WebDriverBy::cssSelector(".product"));
        ...
        foreach ($elements as $element) {
            try {
                $titleElement = $element->findElement(WebDriverBy::className('product-media'));
            ...
            $this->driver
                ...
                ->until(
                    WebDriverExpectedCondition::presenceOfElementLocated(
                        WebDriverBy::cssSelector('.product-details')
                    )
                )->getText();
        } catch (Exception $exception) {
            //
    ...
    public function parseFullProduct(): RemoteProduct
    {
        ...
        try {
            $product->name = $this->driver
                ->findElement(WebDriverBy::className('product text-center'))
                ->getText();
        } catch (Exception $exception) {
            //
        }
        ...
        try {
            $product->description = $this->driver
                ->findElement(
                    WebDriverBy::cssSelector('.product-label-group')
                )
                ->getText();
        } catch (Exception $exception) {
            //
        ...
        try {
            $product->rating = $this->parseRating(
                $this->driver
                    ->findElement(WebDriverBy::className('product-details'))
                    ->getText()
            );
        } catch (Exception $exception) {
            //
        ...
        try {
            $images = $this->driver->findElements(
                WebDriverBy::cssSelector('.product-wrap')
            );
        ...
        try {
            $product->price = $this->parsePrice(
                $this->driver
                    ->findElement(WebDriverBy::className('top-navbar'))
                    ->getText()
            );
        } catch (Exception $exception) {
            //
